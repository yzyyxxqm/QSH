...
class Model(nn.Module):
    '''
    - paper: "Koopa: Learning Non-stationary Time Series Dynamics with Koopman Predictors" (NeurIPS 2023)
    - paper link: https://papers.nips.cc/paper_files/paper/2023/hash/28b3dc0970fa4624a63278a4268de997-Abstract-Conference.html
    - code adapted from: https://github.com/thuml/Time-Series-Library
    '''
    def __init__(self, configs: ExpConfigs, dynamic_dim=128, hidden_dim=64, hidden_layers=2, num_blocks=3, multistep=False):
        """
        ...
        self.input_len = configs.seq_len_max_irr or configs.seq_len # equal to seq_len_max_irr if not None, else seq_len
        ...
        if self.pred_len > self.seq_len:
            logger.error(f"Koopa will encounter error when {self.pred_len=} > {self.seq_len=}")
        ...
        if configs.task_name == "classification":
            logger.exception("Koopa does not support classification task!", stack_info=True)
            exit(1)
