...
class Model(nn.Module):
    """
    - paper: "NHITS: Neural Hierarchical Interpolation for Time Series Forecasting" (AAAI 2023)
    - paper link: https://ojs.aaai.org/index.php/AAAI/article/view/25854
    - code adapted from: https://github.com/Nixtla/neuralforecast
    """
    def __init__(self, configs: ExpConfigs):
        ...
        n_mlp_units=[[configs.d_model, configs.d_model], [configs.d_model, configs.d_model], [configs.d_model, configs.d_model]] # 512
        ...
        self.n_time_in = configs.seq_len_max_irr or configs.seq_len
        self.label_len = configs.label_len
        self.n_time_out = configs.pred_len_max_irr or configs.pred_len
    ...
    def forward(
        ...
        **kwargs
        ...
        Y_LEN = self.pred_len
        if x_mark is None:
            x_mark = repeat(t.arange(end=x.shape[1], dtype=x.dtype, device=x.device) / x.shape[1], "L -> B L 1", B=x.shape[0])
        if y is None:
            if self.configs.task_name in ["short_term_forecast", "long_term_forecast", "imputation"]:
                logger.warning(f"y is missing for the model input. This is only reasonable when the model is testing flops!")
            y = t.ones((BATCH_SIZE, Y_LEN, ENC_IN), dtype=x.dtype, device=x.device)
        if y_mark is None:
            y_mark = repeat(t.arange(end=y.shape[1], dtype=y.dtype, device=y.device) / y.shape[1], "L -> B L 1", B=y.shape[0])
        if y_mask is None:
            y_mask = t.ones_like(y, device=y.device, dtype=y.dtype)
        if y_class is None:
            if self.configs.task_name == "classification":
                logger.warning(f"y_class is missing for the model input. This is only reasonable when the model is testing flops!")
            y_class = t.ones((BATCH_SIZE), dtype=x.dtype, device=x.device)

        x_mark[x_mark == 1] = 0.9999 # cannot process value == 1 in mark
        y_mark[y_mark == 1] = 0.9999

        x_dec = t.cat([x[:, -self.label_len:, :], t.zeros_like(y).float()], dim=1).float().to(x.device)
        x_mark_dec = t.cat([x_mark[:, -self.label_len:, :], y_mark], dim=1).float().to(x_mark.device)
        return_decomposition = False
        # END adaptor

        # insample
        insample_y = x.permute([0, 2, 1]).reshape([-1, self.n_time_in])
        insample_x_t = x_mark.permute([0, 2, 1]).reshape([-1, self.n_time_in])
