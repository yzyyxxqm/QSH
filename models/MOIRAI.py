...
class Model(nn.Module):
    ...
    def __init__(self, configs: ExpConfigs, **kwargs):
        ...
        if self.configs.task_name in ["long_term_forecast", "short_term_forecast"]:
            ...
            self.n_patch_all: int = math.ceil(configs.seq_len / configs.patch_len) # pad pred_len to times of patch_len
    ...
    def forward_all_variate(
        ...
        if self.configs.task_name in ["short_term_forecast", "long_term_forecast"]:
            ...
            observed_mask = self.rearrange_and_pad_multivariate_tensor(x_mask + y_mask).bool()
            prediction_mask = self.rearrange_and_pad_multivariate_tensor(y_mask).sum(dim=-1).bool()
            variate_id = self.rearrange_and_pad_multivariate_tensor(variate_id).sum(dim=-1).int()

            output = self.MoiraiModule(
                target=self.rearrange_and_pad_multivariate_tensor(x),
                observed_mask=observed_mask,
                sample_id=sample_id,
                time_id=self.rearrange_and_pad_multivariate_tensor(
                    repeat(
            ...
            target = self.rearrange_and_pad_multivariate_tensor(y)
