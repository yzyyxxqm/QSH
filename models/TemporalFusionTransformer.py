...
class Model(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        if x_mark is None:
            # if mark contains unexpected 0, it may cause unexpected error, thus force overwrite
        ...
        dec_out = self.forecast(x, x_mark, x_dec, x_mark_dec)  # [B,pred_len,C]
        dec_out = torch.cat([torch.zeros_like(x), dec_out], dim=1)

        f_dim = -1 if self.configs.features == 'MS' else 0
        return {
            "pred": dec_out[:, -self.pred_len:, f_dim:],
            "true": y[:, -self.pred_len:, f_dim:],
            "mask": y_mask[:, -self.pred_len:, f_dim:]
        }
...
class TFTEmbedding(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        return static_input, observed_input, observed_input
...
class InterpretableMultiHeadAttention(nn.Module):
    def __init__(self, configs: ExpConfigs):
        ...
        example_len = (configs.seq_len_max_irr or configs.seq_len) + configs.pred_len
