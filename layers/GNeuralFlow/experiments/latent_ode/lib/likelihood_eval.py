...
def compute_binary_CE_loss(label_predictions, mortality_label):
    ...
    if len(idx_not_nan) == 0.:
        ...
        ce_loss = torch.Tensor(0.).to(mortality_label)

    label_predictions = label_predictions[:,idx_not_nan]
    mortality_label = mortality_label[idx_not_nan]

    if (torch.sum(mortality_label == 0.) == 0 or torch.sum(mortality_label == 1.) == 0) and torch.cuda.is_available():
...
def compute_masked_likelihood(mu, data, mask, likelihood_func):
    ...
    original code's speed bottleneck is here
    '''
    # Compute the likelihood per patient and per attribute
    n_traj_samples, n_traj, n_timepoints, n_dims = data.size()

    # Preallocate result tensor
    res = torch.zeros((n_traj_samples, n_traj), device=data.device)

    # Vectorized implementation
    for i in range(n_traj_samples):
        for k in range(n_traj):
            # Get the masked data and mu for all dimensions at once
            mask_ik = mask[i, k].bool()
            data_masked = data[i, k][mask_ik]
            mu_masked = mu[i, k][mask_ik]

            # Calculate likelihood for all dimensions simultaneously
            if data_masked.numel() > 0:  # Ensure there is data to process
                log_prob = likelihood_func(mu_masked, data_masked, indices=(i, k))
                # Sum or mean across dimensions
                res[i, k] = log_prob.mean(dim=0)
