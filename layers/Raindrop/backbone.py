...
class BackboneRaindrop(nn.Module):
    def __init__(
        ...
        if self.sensor_wise_mask:
            ...
            assert dim_check % n_heads == 0, f"{dim_check=} must be divisible by {n_heads=}"
    ...
    def forward(
        ...
        mask = mask.squeeze(1).to(device) # (BATCH_SIZE, SEQ_LEN)
        ...
        for unit in range(0, batch_size):
            ...
            edge_index_layer2 = attention_weights[0] # (2, ENC_IN * ENC_IN)
            edge_weights_layer2 = attention_weights[1].squeeze(-1) # (ENC_IN * ENC_IN, 1)
        ...
        # (SEQ_LEN, BATCH_SIZE, ENC_IN * d_ob + d_pe)
