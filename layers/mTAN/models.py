...
class enc_mtan_rnn(nn.Module):
    ...
    def forward(self, x, time_steps):
        ...
        if self.learn_emb:
            key = self.learn_time_embedding(time_steps).to(x.device)
            query = self.learn_time_embedding(self.query.unsqueeze(0).to(x.device)).to(x.device)
        else:
            key = self.fixed_time_embedding(time_steps).to(x.device)
            query = self.fixed_time_embedding(self.query.unsqueeze(0).to(x.device)).to(x.device)
...
class dec_mtan_rnn(nn.Module):
    ...
    def forward(self, z, time_steps):
        ...
        if self.learn_emb:
            query = self.learn_time_embedding(time_steps).to(z.device)
            key = self.learn_time_embedding(self.query.unsqueeze(0).to(z.device)).to(z.device)
        else:
            query = self.fixed_time_embedding(time_steps).to(z.device)
            key = self.fixed_time_embedding(self.query.unsqueeze(0).to(z.device)).to(z.device)
