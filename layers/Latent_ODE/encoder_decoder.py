...
class Encoder_z0_RNN(nn.Module):
    def __init__(self, latent_dim, input_dim, lstm_output_size = 20, 
        ...
        if use_delta_t:
            self.input_dim += 1
        self.gru_rnn = GRU(self.input_dim, self.gru_rnn_output_size)
    ...
    def forward(self, data, time_steps, run_backwards = True):
        ...
        if self.use_delta_t:
            ...
            delta_t = torch.cat((delta_t, torch.zeros(1)))
...
class Encoder_z0_ODE_RNN(nn.Module):
    ...
    def forward(self, data, time_steps, run_backwards = True, save_info = False):
        ...
        if len(time_steps) == 1:
            prev_y = torch.zeros((1, n_traj, self.latent_dim))
            prev_std = torch.zeros((1, n_traj, self.latent_dim))

            xi = data[:,0,:].unsqueeze(0)

            last_yi, last_yi_std = self.GRU_update(prev_y, prev_std, xi)
        else:
            last_yi, last_yi_std, _, _ = self.run_odernn(
    ...
    def run_odernn(self, data, time_steps, 
        ...
        # if minimum_step <= 1e-3:
        #     # prevent value been too small
        #     minimum_step = 1e-3
