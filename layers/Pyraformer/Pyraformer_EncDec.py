...
class Encoder(nn.Module):
    ...
    def __init__(self, configs: ExpConfigs, window_size, inner_size):
        super().__init__()
        ...
        self.mask, self.all_size = get_mask(
            self.seq_len, window_size, inner_size)
...
class Bottleneck_Construct(nn.Module):
    ...
    def forward(self, enc_input):
        temp_input = self.down(enc_input).permute(0, 2, 1) # (B, d_inner, seq_len)
        all_inputs = []
