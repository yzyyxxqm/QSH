...
class Encoder(nn.Module):
    def __init__(self, dim=41, nkernel=128, n_layers=3, attn_head=4, task_name="short_term_forecast", n_classes=2):
        ...
        self.merge_type = "linear" # ["linear", "parameter"]
        if self.merge_type == "linear":
            self.score_layer = nn.Linear(nkernel, nkernel)
        elif self.merge_type == "parameter":
            # for MIMIC-III & MIMIC-IV
            self.mapping_pool = nn.Parameter((1-2*torch.rand((nkernel, nkernel))))
    ...
    def forward(self, context_x, value, mask, target_value, target_mask, x_repr_var, exp_stage):
        """
        ...
        if self.merge_type == "linear" and x_repr_var is not None:
        ...
        if self.merge_type == "parameter" and x_repr_var is not None:
            channel_embedding = x_repr_var + torch.matmul(channel_embedding, self.mapping_pool)

        if self.task_name in ["short_term_forecast", "long_term_forecast", "imputation", "representation_learning"]:
