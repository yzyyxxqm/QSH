...
class BackboneRaindrop(nn.Module):
    def __init__(
        ...
        self.init_weights()
        self.mapping_pool = nn.Parameter((1-2*torch.rand((max_len,max_len))))
        self.score_layer = nn.Linear(d_model, d_model)
    ...
    def forward(
        ...
        ----------
        X :
            The input tensor of shape (batch_size, max_len, n_feature).
        ...
        """
        src = rearrange(X, "BATCH_SIZE SEQ_LEN ENC_IN -> SEQ_LEN BATCH_SIZE ENC_IN")
        times = rearrange(timestamps, "BATCH_SIZE SEQ_LEN -> SEQ_LEN BATCH_SIZE")

        device = src.device
        max_len, batch_size = src.shape[0], src.shape[1]

        src = torch.repeat_interleave(src, self.d_ob, dim=-1) # (SEQ_LEN, BATCH_SIZE, ENC_IN * d_ob)
        h = F.relu(src * self.R_u) # (SEQ_LEN, BATCH_SIZE,  ENC_IN * d_ob)
        pe = self.pos_encoder(times).to(device) # (SEQ_LEN, BATCH_SIZE, d_pe)
        h = self.dropout_layer(h)

        mask = torch.arange(max_len)[None, :].to(device) >= (lengths[:, None])
        mask = mask.squeeze(1) # (BATCH_SIZE, SEQ_LEN)

        x = h

        # adj matrix with 1s along the diagonal
        # actually default to fully connected adj
        ...
        if x_repr_time is not None:
            score = torch.sigmoid(self.score_layer(x_repr_time))
            output = output + (score * x_repr_time).permute(1, 0, 2)
