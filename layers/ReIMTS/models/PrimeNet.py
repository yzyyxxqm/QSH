...
from layers.ReIMTS.layers.PrimeNet.models import (
    TimeBERTConfig,
    ...
    def __init__(
        ...
        **kwargs
        ...
        if configs.task_name in ['long_term_forecast', 'short_term_forecast', "imputation"]:
            self.loss_fn_finetune = MSE(configs)
        elif configs.task_name == "classification":
            '''
            used at train_stage == 2, i.e., finetuning
            '''
            self.loss_fn_finetune = CrossEntropyLoss(configs)
        ...
        self.n_patch_all: int = 1 # force overwrite as 1 when used as backbone, since no need to split inside backbone
        if self.configs.task_name in ["short_term_forecast", "long_term_forecast"]:
            self.patch_len = self.seq_len + self.pred_len
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.configs.task_name in ["short_term_forecast", "long_term_forecast"]:
            ...
            observed_data = x.reshape(BATCH_SIZE, self.n_patch_all, -1, ENC_IN)
            observed_tp = x_mark.reshape(BATCH_SIZE, self.n_patch_all, -1, 1)[:, :, :, 0]
            observed_mask = x_mask.reshape(BATCH_SIZE, self.n_patch_all, -1, ENC_IN)
            interp_mask = y_mask.reshape(BATCH_SIZE, self.n_patch_all, -1, ENC_IN)
        elif self.configs.task_name in ["classification"]:
            observed_data = x.reshape(BATCH_SIZE, self.n_patch_all, -1, ENC_IN)
            observed_tp = x_mark.reshape(BATCH_SIZE, self.n_patch_all, -1, 1)[:, :, :, 0]
            observed_mask = x_mask.reshape(BATCH_SIZE, self.n_patch_all, -1, ENC_IN)
            interp_mask = torch.zeros_like(observed_mask, device=observed_mask.device)
        else:
            raise NotImplementedError()
        # END adaptor
        ...
        if self.configs.task_name in ["short_term_forecast", "long_term_forecast", "imputation"]:
            ...
            return {
                ...
                "mask": y_mask[:, :, f_dim:],
                "loss": loss,
                "pred_repr_time": out["last_hidden_state"][:, :SEQ_LEN]
            }
            ...
            return {
                "pred_class": pred,
                "true_class": y_class,
                "pred_repr_time": out["last_hidden_state"],
                "loss": loss
            }
        else:
            raise NotImplementedError