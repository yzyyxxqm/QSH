...
from layers.ReIMTS.layers.Raindrop import BackboneRaindrop
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        n_layers = self.configs.n_layers # WARNING: unused in ReIMTS_Raindrop
    ...
    def forward(
        ...
        **kwargs
        ...
        pred_repr_time = representation.permute(1, 0, 2)

        if self.current_level != self.n_fractal_levels - 1:
            return {
                "pred_repr_time": pred_repr_time
            }
        else:
            ...
            if self.configs.task_name == "classification":
                ...
                output = rearrange(representation, "SEQ_LEN BATCH_SIZE (ENC_IN D) -> BATCH_SIZE SEQ_LEN ENC_IN D", BATCH_SIZE=BATCH_SIZE, ENC_IN=self.n_features, D=self.d_ob)
                pred = self.mlp_forecast(self.unpatchify_observation_embedding(output)).squeeze(-1)
                # BEGIN adaptor
    ...
    def unpatchify_observation_embedding(self, x):
        return rearrange(x, "(B N_PATCH) SEQ_LEN ENC_IN D -> B (N_PATCH SEQ_LEN) ENC_IN D", N_PATCH=self.total_scale_factor)