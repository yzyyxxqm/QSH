...
from layers.ReIMTS.layers.mTAN.models import (
    create_classifier,
    dec_mtan_rnn,
    enc_mtan_rnn,
)
...
class Model(nn.Module):
    ...
    def __init__(self, configs: ExpConfigs, **kwargs):
        ...
        if configs.task_name in ["short_term_forecast", "long_term_forecast", "imputation"]:
            self.mapping_pool = nn.Parameter((1-2*torch.rand((num_ref_points // 2, num_ref_points)))) # DEBUG: // 2 should be replaced with the actual ratio
            self.score_layer = nn.Linear(self.latent_dim, self.latent_dim)
        else:
            raise NotImplementedError()
    ...
    def forward(
        ...
        **kwargs
        ...
        if x_repr_time is not None:
            ...
            z0 = z0 + score * x_repr_time_mapped
        pred_x = self.dec(
            z0,
