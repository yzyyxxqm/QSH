...
def spawner(x, labels, sigma=0.05, verbose=0):
    ...
    import data.dependencies.MTS_Dataset.utils.dtw as dtw
...
def run_augmentation(x, y, configs):
    print("Augmenting %s"%configs.data)
    np.random.seed(configs.seed)
    x_aug = x
    y_aug = y
    if configs.augmentation_ratio > 0:
        augmentation_tags = "%d"%configs.augmentation_ratio
        for n in range(configs.augmentation_ratio):
            x_temp, augmentation_tags = augment(x, y, configs)
            x_aug = np.append(x_aug, x_temp, axis=0)
            y_aug = np.append(y_aug, y, axis=0)
            print("Round %d: %s done"%(n, augmentation_tags))
        if configs.extra_tag:
            augmentation_tags += "_"+configs.extra_tag
    else:
        augmentation_tags = configs.extra_tag
    return x_aug, y_aug, augmentation_tags

def run_augmentation_single(x, y, configs):
...
def augment(x, y, configs):
    import data.dependencies.MTS_Dataset.utils.augmentation as aug
    augmentation_tags = ""
    if configs.jitter:
        x = aug.jitter(x)
    ...
    if configs.permutation:
        x = aug.permutation(x)
        augmentation_tags += "_permutation"
    if configs.randompermutation:
