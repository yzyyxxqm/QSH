...
from data.dependencies.tsdm.datasets import MIMIC_IV_Bilos2021 as MIMIC_IV_Dataset
...
def mimic_collate(batch: list[Sample]) -> Batch:
    ...
    x_time.append(torch.zeros(100))
    y_time.append(torch.zeros(3))
    x_vals.append(torch.zeros(100, 100))
    y_vals.append(torch.zeros(3, 100))
    x_mask.append(torch.zeros(100, 100))
    y_mask.append(torch.zeros(3, 100))
    ...
    return {
        "observed_data": torch.nan_to_num(x_),
        "observed_mask": mask_observe,
        "mask_train": mask_train,
        "time_steps": x_time,
        "feature_representation": torch.tensor([]),
    }
