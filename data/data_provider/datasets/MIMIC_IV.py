...
from data.dependencies.tsdm.tasks.mimic_iv_bilos2021 import Sample
...
class Data(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        '''
        warpper for MIMIC IV Bilos2021 dataset implemented in tsdm
    ...
    def preprocess(self):
        ...
        """
        if self.seq_len + self.pred_len > 2880:
            logger.exception(f"{self.seq_len+self.pred_len=} is too large. Expect the value smaller than 2880")
            exit(1)
            ...
            for sample in dataset:
                ...
                if self.configs.collate_fn == "collate_fn_patch":
                    ...
                    for i in range(n_patch):
                        observations = x_mark < ((i + 1) * PATCH_LEN / 2880)
                    ...
                    for j in range(n_patch_y):
                        observations = y_mark < (((n_patch + j + 1) * PATCH_LEN) / 2880)
